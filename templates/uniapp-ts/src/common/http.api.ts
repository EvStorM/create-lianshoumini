...
const install = (Vue: Vue, vm: VM) => {
  ...
  // 创建post方式的api请求
  const createPostApi = (pash: string) => {
    return (params = {}) =>
      vm.$u
        .post('run' + pash, params)
        .then((v: any) => v)
        .catch((e: any) => {
          if (e.statusCode == 401) {
            // 如果token过期报401
  ...
  // 创建文件上传接口
  let uploadImg = (filePath: string, formData = {}, name = 'file') => {
  ...
  // 16进制色彩转RGBA
  vm.$u.sixteenToRGBA = (hex: string, alpha: string) => {
